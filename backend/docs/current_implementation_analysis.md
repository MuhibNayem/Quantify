...
### Stock Operations (`stock.go`, `inventory.go`)
...
### Product Management (`products.go`)
...
### Categories & Suppliers (`categories.go`, `suppliers.go`)
...
### Current Reports (`reports.go`, `reporting_service.go`)
- ✅ **Sales Trends Report**:
...
### Demand Forecasting (`forecasting_service.go`)
...
- ✅ **Forecast Storage**: `DemandForecast` table with period (30_DAYS, 90_DAYS)

### Reorder Management (`replenishment.go`, `replenishment_service.go`)
...
### Purchase Orders (`replenishment.go`)
...
### Purchase Returns (`replenishment.go`)
...
### Stock Alerts (`alerts.go`)
- ✅ **Alert Types**: LOW_STOCK, OUT_OF_STOCK, OVERSTOCK, EXPIRY_ALERT
- ✅ **Per-Product Thresholds**: Configurable via `ProductAlertSettings`
- ✅ **Auto-Triggering**: Background function `CheckAndTriggerAlerts()`
- ✅ **Alert Resolution**: Mark alerts as RESOLVED
- ✅ **Expiry Alerts**: Checks batches expiring within `ExpiryAlertDays`

### Notifications (`notifications.go`, `notification_subscriptions.go`)
- ✅ **In-App Notifications**: `Notification` model with read/unread status
- ✅ **Real-time via WebSocket**: Push notifications to connected clients
- ✅ **User Preferences**: `UserNotificationSettings` for email/SMS toggles
- ✅ **Role-Based Subscriptions**: `AlertRoleSubscription` (e.g., "Manager gets LOW_STOCK alerts")
...
### Customer Management (`crm.go`, `crm_service.go`)
...
### Time Clock (`time_tracking.go`, `time_tracking_service.go`)
...
- ✅ **Refresh Token Storage**: `RefreshToken` table with expiry

### Authorization (`role_handler.go`, `role_service.go`)
- ✅ **Role-Based Access Control (RBAC)**: `Role` model with permissions
- ✅ **Predefined Roles**: From `domain/roles.go` (Admin, Manager, Cashier, etc.)
...
### Configuration (`settings.go`, `settings_service.go`)
...
- **Order** (from `domain/orders.go` - assumed POS sales)
- **Transaction** (payments)
- **LoyaltyAccount**
- **Job** (async task tracking)
- **Notification**
- **TimeClock**
- **SystemSetting** (from `domain/settings.go`)
...
### Caching (`repository/redis.go`)
- ✅ **Redis**: Cache report data, session tokens

### Database (`repository/database.go`)
...
### Testability
- ⚠️ Limited test coverage observed (only `return_test.go` visible)
