...
func ListProducts(c *gin.Context) {
	...
	var total int64
	repository.DB.Model(&domain.Product{}).Count(&total)
...
func GetProduct(c *gin.Context) {
	...
	if err := repository.DB.Preload("Category").Preload("SubCategory").Preload("Supplier").Preload("Location").First(&product, id).Error; err != nil {
...
func UpdateProduct(c *gin.Context) {
	...
	// Update fields
	if err := repository.DB.Model(&product).Updates(domain.Product{
		Name:          req.Name,
		Description:   req.Description,
		CategoryID:    req.CategoryID,
		SubCategoryID: req.SubCategoryID,
...
func DeleteProduct(c *gin.Context) {
	...
	repository.DB.Model(&domain.Batch{}).Where("product_id = ?", id).Count(&batchCount)
	if batchCount > 0 {
	...
	repository.DB.Model(&domain.StockAdjustment{}).Where("product_id = ?", id).Count(&adjustmentCount)
...
func ArchiveProduct(c *gin.Context) {
	...
	if err := repository.DB.Model(&product).Update("Status", req.Status).Error; err != nil {
